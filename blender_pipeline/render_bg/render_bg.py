...
def adjust_lighting_to_target_brightness(target_brightness=0.4, preview_resolution=(128, 72), camera_list=None):
    ...
    # 【关键改动】给文件系统一点时间来完成写入操作。
    # 即使Blender报告“已保存”，文件句柄也可能没有被完全关闭和刷新。
    # 这个延时也能间接缓解因版本不匹配导致的不稳定。
    time.sleep(0.5) 
...
# 之前的脚本在这里会定义物体的移动路径，并为每一帧、每一个相机创建一个渲染任务。
...
### === 开始渲染背景图 === ###
print(f"🎬 开始为 {len(camera_list)} 个相机视角渲染背景图...")
