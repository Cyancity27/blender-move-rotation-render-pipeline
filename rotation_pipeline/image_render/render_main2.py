# 脚本1: 主图渲染脚本 (render_main.py) - 断点续传优化版（适配旋转+平移版Mask）
# 功能：根据【已筛选好】的Mask文件列表，智能跳过已渲染的主图，仅渲染缺失的部分，并追加生成CSV日志。
# 适配点：支持 rot_init 与 rotcwDDD_stepNN 命名；与 Mask 完全一致的步长/旋转规则；逐帧贴地，确保对齐。
...
if "indoor" in scene_lower:
    ...
    move_step = [0.10, 0.10, 0]
elif ("outdoor6" in scene_lower) or ("outdoor12" in scene_lower) or ("outdoor5" in scene_lower):
    move_step = [0.6, 0.6, 0]
else:
...
for img_name in tasks_to_render_filenames:
    try:
        ...
        if m_init:
            ...
            # 位置：从初始化点出发，沿世界系 move_step 前进 step_idx 步
            pos = (
